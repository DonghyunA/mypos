...
public class Select_Item_Activity extends Activity {
	private static String IP_ADDRESS = "10.1.31.177";
	private static String TAG = "phptest";
	private String mJsonString;
	//private ArrayList<ListviewItem> mArrayList;
	private static final char[] INITIAL_SOUND = { 'ㄱ', 'ㄲ', 'ㄴ', 'ㄷ', 'ㄸ', 'ㄹ', 'ㅁ', 'ㅂ', 'ㅃ', 'ㅅ', 'ㅆ', 'ㅇ', 'ㅈ', 'ㅉ', 'ㅊ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ' };
	ArrayList<Select_Items> temp_iteminfo = new ArrayList<Select_Items>();
	Select_Item_Adapter adapter = new Select_Item_Adapter();
	...
	private class GetData extends AsyncTask<String, Void, String>{
		...
		protected void onPreExecute() {
			...
			progressDialog = ProgressDialog.show(Select_Item_Activity.this,"Please Wait", null, true, true);
		}
		@Override
			...
			try {
				...
				for(int j=0;j<INITIAL_SOUND.length;j++)
				{
					for(int k=0;k<jsonArray.length();k++)
					{
						...
						int i_price = item.getInt(TAG_PRICE);
						String i_date = item.getString(TAG_DATE);
						//구조체에 넣기
						if(j==0)
						{
							adapter.addItem(i_id, i_name, i_producer, i_quality, i_quantity, i_purchased, i_price, i_date);
						}
					...
					{
//       	             // LinearLayout 생성
						LinearLayout ll = new LinearLayout(Select_Item_Activity.this);
						ll.setOrientation(LinearLayout.HORIZONTAL);
						// 버튼 생성

						final Button btn = new Button(Select_Item_Activity.this);
						// setId 버튼에 대한 키값
						btn.setId(j + 1);
